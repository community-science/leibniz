...
const ROOTS: RootTree[] = [
  ...
  },
  {
    ...
    nodes: [
        ...
        rungs: [
          {
            ...
            kind: 'add',
            tag: 'the two entries',
            text: 'Credit is validated bits: the ambient $\\varepsilon$-entropy of the field evolution above persistence, resolved to the residual-certified $\\varepsilon$ (continuous, no gate), replacing the earlier grid-degree-of-freedom count $N_{dof}\\log_2(\\sigma/\\varepsilon_{field})$. Debit is the measured cost of running the operator — a machine-independent operation count today, energy (Joules) as the goal — not a description-length term.',
          },
          {
            level: 2,
            kind: 'add',
            tag: 'one quantity to minimize',
            text: 'The value side has a concrete validated-bit quantity for field prediction; the cost side is the measured operation count (energy as the goal), which the runner attaches and integrates against validated bits over the measured frontier. There is no description-length term to add — parsimony is emergent, not minimized (see Cost).',
          },
          {
            level: 2,
            kind: 'add',
      ...
      {
        id: 'D-cost',
        gist: 'Cost is a measured physical quantity: operations now, energy as the goal.',
        meta: 'Measured, not a description-length penalty. Parsimony is pressured by low cost, not minimized directly.',
        status: 'mixed',
        anchor: 'cost metrology: machine-independent operation count (in code); energy as the goal',
        step: 'cost',
        rungs: [
          {
            level: 1,
            ...
            text: 'Cost is a single measured quantity from the submitted program, not the machine it ran on: a machine-independent operation count under the declared per-op cost model. Parsimony is emergent, not minimized — a flop-cheap but inscrutable memorizer is excluded because its validated bits collapse on harder rungs (the correctness axis), and a simple but expensive brute force is excluded by cost; what survives, generalizing bits at low cost, is the parsimonious explanation.',
          },
          {
            level: 2,
            kind: 'horizon',
            tag: 'open',
            text: 'The real target is energy (Joules), which prices memory as well as compute, so a flop-cheap lookup table is still expensive in moving and holding its bits. How to extend the operation count into a roofline-shaped energy model — pricing bytes moved and resident, with the compute-vs-memory ratio a declared parameter — still needs defining.',
          },
          {
            level: 3,
            kind: 'code',
            tag: '⟂',
            text: 'cost metrology: operation count is in code; the energy model is a direction',
          },
        ],
        verdict: { tone: 'partial', text: 'Operation count is in code; energy is the goal. Description length is rejected as an objective — parsimony is emergent.' },
      },
    ],
  },
];
...
const STEPS: RoadmapStep[] = [
  ...
  },
  {
    ...
    outcome:
      'Extend the machine-independent operation count toward energy (price bytes moved and resident under a roofline model) and keep the score at validated bits per unit cost. There is no description-length term — parsimony stays emergent. The test: two models with identical predictions but different cost score differently.',
  },
  {
    id: 'hierarchy',
